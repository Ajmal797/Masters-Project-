...
def create_feature_based_graph(df, k=5):
    num_nodes = len(df)
...
# Training Function
def train_model(model, graph_data, epochs=20, lr=0.01):
...
def plot_tsne(embeddings, labels, title="t-SNE Visualization", filename="tsne_plot"):
    tsne = TSNE(n_components=2, perplexity=30, random_state=42)
    tsne_results = tsne.fit_transform(embeddings)
    
    plt.figure(figsize=(10, 8))
    scatter = plt.scatter(tsne_results[:, 0], tsne_results[:, 1], c=labels, cmap='jet', alpha=0.7)
    plt.colorbar(scatter, label='Attack Label')
    plt.title(title)
...
# Example of training and testing MPNN
model = MPNN(input_dim=len(feature_cols), hidden_dim=64, output_dim=len(np.unique(merged_df['Attack Label'])))
trained_model = train(model, graph_data, class_weights)
test(trained_model, graph_data)
...
for name, model in models.items():
    ...
    print(f'\nTesting {name}...')
    embeddings = test(trained_model, graph_data)
    
    # Save t-SNE visualization for each model separately
    plot_tsne(embeddings, graph_data.y.cpu().numpy(), title=f"t-SNE Visualization - {name}", filename=f"tsne_{name}")
